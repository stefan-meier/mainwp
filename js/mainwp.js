...
var wpgeneral_bulkMaxThreads = 3;
var wpgeneral_bulkCurrentThreads = 0;
var wpgeneral_bulkTotalThreads = 0;
var wpgeneral_bulkFinishedThreads = 0;

mainwp_wpgeneral_save_settings_onclick = function() {       
    
    if (!confirm('Are you sure?'))
        return false;
    
    jQuery( '#selected_sites' ).removeClass('form-invalid');
    jQuery( '#mainwp-wpgs-table-settings' ).removeClass('mainwp-input-invalid');
    
    var errors = [];
    var selected_sites = [];
    var selected_groups = [];
    
    if (jQuery("input[name='checked_bsm_settings[]']:checked").length == 0) {
        errors.push( __( 'Please select settings.' ) );    
        jQuery( '#mainwp-wpgs-table-settings' ).addClass('mainwp-input-invalid');
    }
    
    if (jQuery( '#wpgeneral_settings_select_sites_wrap #select_by' ).val() == 'site') {
            jQuery( "input[name='selected_sites[]']:checked" ).each(function (i) {
                    selected_sites.push( jQuery( this ).val() );
            });
            if (selected_sites.length == 0) {
    ...
    if (errors.length > 0) {
            setHtml( '#mwp-bsm-general-settings-error-box', errors.join( '<br />' ), false );
            return false;
    } else {
            jQuery( '#mwp-bsm-general-settings-error-box' ).html( "" );
            jQuery( '#mwp-bsm-general-settings-error-box' ).hide();
    }
    return true;                
}

mainwp_wpgeneral_actions_start = function() {
    wpgeneral_bulkTotalThreads = jQuery('.siteItemProcess[status=queue]').length;
    mainwp_wpgeneral_actions_start_next();
}

mainwp_wpgeneral_actions_start_next = function() {
	while ((objProcess = jQuery( '.siteItemProcess[status=queue]:first' )) && (objProcess.length > 0) && (wpgeneral_bulkCurrentThreads < wpgeneral_bulkMaxThreads)) {
            objProcess.attr( 'status', 'processed' );                    
            mainwp_wpgeneral_save_settings_start_specific( objProcess );                                     
	}
	if (wpgeneral_bulkFinishedThreads > 0 && wpgeneral_bulkFinishedThreads == wpgeneral_bulkTotalThreads) {          
            jQuery( '#mainwp_wpgeneral_saving_content' ).append( '<div class="mainwp_info-box">' + __( "Finished." ) + '</div>' + '<p><a class="button-primary" href="admin.php?page=mainwp-wp-general-settings">Return the Wordpress General Settings</a></p>' );                   
	} 
}

mainwp_wpgeneral_save_settings_start_specific = function(objProcess) {  
        var loadingEl = objProcess.find( 'i' );
	var statusEl = objProcess.find( '.status' );
        loadingEl.show();
        statusEl.hide();
        wpgeneral_bulkCurrentThreads++;
	jQuery.post(ajaxurl, {
                action: 'mainwp_wpgeneral_settings_save',
                siteId: objProcess.attr( 'site-id' ),
                nonce: mainwp_ajax_nonce
	}, function(data) {		
                if (data) {                        
                        if (data.error) {
                                statusEl.css( 'color', 'red' );
                                statusEl.html( '<i class="fa fa-exclamation-circle"></i> ' + data.error );
                        } else if (data.result) {
